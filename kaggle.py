...
class conv_layer(object):
    def __init__(self, input_x, in_channel, out_channel, kernel_shape, rand_seed, index=0):
        ...
        print(input_x)
        print(input_x.shape)
        print(len(input_x.shape) == 4)
        print(input_x.shape[1] == input_x.shape[2])
        print(input_x.shape[3], in_channel, input_x.shape[3] == in_channel)
...
def LeNet(input_x, input_y,
    ...
    conv_layer_0 = conv_layer(input_x=input_x,
                              ...
                              rand_seed=seed)
    print("Pass0",conv_layer_0.output().shape)

    pooling_layer_0 = max_pooling_layer(input_x=conv_layer_0.output(),
                                        k_size=pooling_size[0],
                                        padding="VALID")
    print("Pass1",pooling_layer_0.output().shape)

    # norm_layer_0 = batch_norm_layer(pooling_layer_0.output())
    norm_layer_0 = Local_Response_norm_layer(pooling_layer_0.output())
    print("Pass2",norm_layer_0.output().shape)

    conv_layer_1 = conv_layer(input_x=norm_layer_0.output(),
                              in_channel=norm_layer_0.output().shape[3],
                              out_channel=conv_featmap[1],
                              kernel_shape=conv_kernel_size[1],
                              rand_seed=seed,
                              index=1)
    print("Pass3",conv_layer_1.output().shape)

    pooling_layer_1 = max_pooling_layer(input_x=conv_layer_1.output(),
                                        k_size=pooling_size[1],
                                        padding="VALID")
    print("Pass4",pooling_layer_1.output().shape)

    # norm_layer_1 = batch_norm_layer(pooling_layer_1.output())
    norm_layer_1 = Local_Response_norm_layer(pooling_layer_1.output())
    print("Pass5",norm_layer_1.output().shape)

    # flatten
    pool_shape = norm_layer_1.output().get_shape()
    ...
    fc_layer_0 = fc_layer(input_x=flatten,
                          ...
                          index=0)
    print("Pass6",fc_layer_0.output().shape)

    fc_layer_1 = fc_layer(input_x=fc_layer_0.output(),
                          in_size=fc_layer_0.output().shape[1],
...
def training(X_train, y_train, X_val, y_val, 
    ...
    with tf.name_scope('inputs'):
        xs = tf.placeholder(shape=[None, img_len, img_len, 3], dtype=tf.float32)
